...
uint8 ULPPMarchingMeshComponent::GetMarchingID ( const FIntVector& Offset ) const
{
	if ( IsDataComponentValid ( ) )
	{
		...
		for ( int32 MarchingIndex = 0 ; MarchingIndex < 8 ; ++MarchingIndex )
		{
			...
			const FIntVector   DataIndex      = IndexTranslator->AddOffsetToGridIndex ( FIntVector ( RegionIndex , ChunkIndex , 0 ) , Offset + MarchingOffset );
...
bool ULPPMarchingMeshComponent::UpdateRender ( )
{
	...
	TBitArray < > CacheDataList = TBitArray ( false , CacheDataIndex );
	{
		...
		for ( int32 SolidIndex = 0 ; SolidIndex < CacheDataIndex ; ++SolidIndex )
		{
			...
			const FIntVector CheckIndex  = IndexTranslator->AddOffsetToGridIndex ( FIntVector ( RegionIndex , ChunkIndex , 0 ) , CheckOffset );
...
void ULPPMarchingMeshComponent::UpdateDistanceField ( )
{
	...
	// Fill Mesh Hole
	{
		...
		for ( int32 DataIndex = 0 ; DataIndex < DataNum ; ++DataIndex )
		{
			...
			const FIntVector VoxelDataIndex = IndexTranslator->AddOffsetToGridIndex ( FIntVector ( RegionIndex , ChunkIndex , 0 ) , VoxelPos );
			...
			if ( SelfVoxelTag.MatchesTag ( HandleTag ) )
			{
				...
				{
					const FIntVector& TargetIndex = IndexTranslator->AddOffsetToGridIndex ( VoxelDataIndex , LFPMarchingRenderConstantData::FaceDirection [ FaceDirectionIndex ].Up );
